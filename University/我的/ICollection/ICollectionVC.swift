...
class ICollectionVC: UIViewController {
    ...
    private func getCollections() {
        ...
        Alamofire.request(baseURL + "/api/v1/user/\(GlobalData.sharedInstance.userID)/collections/essay/split?page=1", headers: headers).responseJSON { [weak self] response in
            if let self = self {
    ...
    private func loadMore() {
        currentPage += 1
        MBProgressHUD.showAdded(to: view, animated: true)
        Alamofire.request(baseURL + "/api/v1/user/\(GlobalData.sharedInstance.userID)/collections/essay/split?page=\(currentPage)", headers: headers).responseJSON { [weak self]  response in
