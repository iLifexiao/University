...
class ReadingVC: UIViewController {
    ...
    private func getBooks() {
        ...
        Alamofire.request(baseURL + "/api/v1/book/split?page=1", headers: headers).responseJSON { [weak self] response in
    ...
    private func loadMore() {
        currentPage += 1
        MBProgressHUD.showAdded(to: view, animated: true)
        Alamofire.request(baseURL + "/api/v1/book/split?page=\(currentPage)", headers: headers).responseJSON { [weak self]  response in
            guard let self = self else {
                return
            }
            switch response.result {
